...
Similarly, we will implement another [PSP22](/OpenBrush/smart-contracts/PSP22) token 
which will represent the ownership of assets available by the smart contract 
to be lent. In this token, we will need [PSP22Metadata](/OpenBrush/smart-contracts/PSP22/extensions/metadata) 
and we will also need to mint and burn this token. We only want our contract(lending contract) to 
perform these actions, so we will also add the [Ownable](/OpenBrush/smart-contracts/ownable) extension.
...
In addition to the dependencies imported in the [PSP22](/OpenBrush/smart-contracts/PSP22)
documentation, we will also add the `ownable` dependency the same way as in the
[ownable](/OpenBrush/smart-contracts/ownable) documentation. We will be using `SharesContract`
